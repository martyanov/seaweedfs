...
func runUpload(cmd *Command, args []string) bool {
	...
	defaultReplication, err := readMasterConfiguration(grpcDialOption, rpc.ServerAddress(*upload.master))
	if err != nil {
	...
	if len(args) == 0 {
		...
		err = filepath.Walk(util.ResolvePath(*upload.dir), func(path string, info os.FileInfo, err error) error {
			if err == nil {
				if !info.IsDir() {
					...
					results, e := operation.SubmitFiles(func() rpc.ServerAddress { return rpc.ServerAddress(*upload.master) }, grpcDialOption, parts, *upload.replication, *upload.collection, *upload.dataCenter, *upload.ttl, *upload.diskType, *upload.maxMB, *upload.usePublicUrl)
		...
		results, err := operation.SubmitFiles(func() rpc.ServerAddress { return rpc.ServerAddress(*upload.master) }, grpcDialOption, parts, *upload.replication, *upload.collection, *upload.dataCenter, *upload.ttl, *upload.diskType, *upload.maxMB, *upload.usePublicUrl)
		if err != nil {
			fmt.Println(err.Error())
			return false
		}
...
func readMasterConfiguration(grpcDialOption grpc.DialOption, masterAddress rpc.ServerAddress) (replication string, err error) {
	err = rpc.WithMasterClient(false, masterAddress, grpcDialOption, false, func(client master_pb.SeaweedClient) error {
