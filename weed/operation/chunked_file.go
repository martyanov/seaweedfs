...
func NewChunkedFileReader(chunkList []*ChunkInfo, master rpc.ServerAddress, grpcDialOption grpc.DialOption) *ChunkedFileReader {
	var totalSize int64
...
func (cf *ChunkedFileReader) WriteTo(w io.Writer) (n int64, err error) {
	...
	for ; chunkIndex < len(cf.chunkList); chunkIndex++ {
		...
		fileUrl, jwt, lookupError := LookupFileId(func() rpc.ServerAddress {
			return cf.master
